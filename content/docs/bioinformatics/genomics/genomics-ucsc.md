...
```
구성은 다음과 같다.
- 1, 2, 3번째 컬럼은 염색체, 시작위치, 종료위치를 나타낸다. 
...
BED 파일의 기본 형식은 첫번째, 두번째, 세번째 컬럼에 각각 염색체, 시작위치, 종료위치 정보를 갖는 것이다. 이 형식만 만족해도 .bed 확장자 파일 Integrative Genomics Viewer (IGV) 상에 로드가 가능하다. 
