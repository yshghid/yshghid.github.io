...
이와 같은 중요한 지적에 깊이 감사드립니다. 본 코멘트는 저희가 왜 돌연변이 다양성에 주목하게 되었는지를 명확히 설명할 수 있는 기회를 제공해 주었습니다. 아래에 저희 접근법의 이론적 근거, 생물학적 맥락, 선행 연구들과의 연계성, 그리고 기존 빈도 기반 방법과의 비교를 바탕으로 자세히 설명드립니다.
...
돌연변이 다양성(mutation diversity)은 일반적으로 샤논 엔트로피(Shannon entropy) 혹은 유사한 지표를 통해 정량화되며, 특정 위치에서 서로 다른 돌연변이들의 분포와 균등성을 반영합니다. 엔트로피가 높은 위치는 다수의 대체 변이가 발생하고 있다는 것을 나타내며, 이는 기능적 유연성 또는 양성 선택이 작용하는 지점을 의미할 수 있습니다.

다양성 분석은 다음과 같은 장점을 제공합니다:

- 면역적으로 중요한 유연한 영역 탐지: 엔트로피가 높은 위치는 종종 에피토프(epitope), 표면 노출 부위, 면역 회피 부위와 겹치며, 선택 압력이 보존보다는 다양성을 선호하는 영역입니다.
- 잠재적 변이 발생지 조기 탐지: 지배적인 변이는 아직 없지만, 다양한 돌연변이가 발생하고 있는 위치는 미래의 변이 우려 변이(VOC)가 출현할 수 있는 ‘진화의 실험실’일 수 있습니다.
- 계통 효과 최소화: 엔트로피는 계통 유래 효과나 founder effect에 덜 민감하며, 특정 위치에서의 진화 경향을 더 잘 반영합니다.
...
이러한 결과들은 돌연변이 다양성이 단지 이론적 개념이 아니라, 실제 바이러스 기능과 진화 방향성을 이해하는 데 핵심적인 도구임을 보여줍니다.

### 우리의 제안: H-score (Mutation Entropy × Frequency)

저희는 돌연변이의 **발생 빈도**와 **다양성**을 동시에 반영할 수 있는 **H-score**를 제안합니다. 이는 특정 위치에서 돌연변이가 발생했을 때의 조건부 엔트로피를 계산하고, 그 위치의 돌연변이 빈도와 곱하여 계산합니다.

- **엔트로피는 돌연변이된 염기만 고려:** 보존된 참조 염기는 제외되므로, 실제 변이 분포의 다양성만 반영합니다.
- **빈도와 다양성의 통합적 반영:** H-score는 자주 변이되며 동시에 다양한 방식으로 변이된 위치를 강조합니다. 이는 기능적 중요성과 진화 압력이 동시에 작용하는 진정한 핫스팟을 반영합니다.

이러한 이중 초점 방식은 다음을 가능하게 합니다:

- 빈도 기반 필터링으로는 놓치는 **적응력 높은 영역(c315, c442 등)**을 탐지.
- **계통 정의 변이와 무관한 기능적 핫스팟**을 우선순위에 올림.
- 중증도 및 면역 활성화 등 **임상적 지표와의 연관성**이 있는 위치를 식별.
