...
bookHidden: true
title: "netMHCpan: #1 환자 시퀀스 allprot.fasta 생성
---

# netMHCpan: #1 환자 시퀀스 allprot.fasta 생성
...
```python
sequence_df = make_sequence_df()
cluster_df = pd.read_csv('/data/home/ysh980101/2312/Data/MHC/clusters.tsv', sep='\t')
...
# fasta 파일 저장 경로 설정
output_dir = f"/data/home/ysh980101/2411/data-mhc/{cur_cluster}/"
...
#function make_sequence_df

```python
def make_sequence_df():
    # 참조 시퀀스 파일 불러오기 및 컬럼 이름 변경
    ref_sequence = pd.read_csv("/data3/projects/2020_MUTCLUST/Data/Annotation/Codon/Reference/new_reference_codon.csv", index_col=0)
    ref_sequence.rename(columns={'sequence': 'reference'}, inplace=True)

    # 코돈 파일 목록 불러오기
    file_list = os.listdir("/data3/projects/2020_MUTCLUST/Data/Projects/COVID19/Sequence/Raw/Codon")
    patient_list = [f.split('.')[0] for f in file_list]

    # 메타데이터 파일 불러오기
    meta = pd.read_csv("/data/home/ysh980101/2408/data/covid19-meta.csv")
    ...
    for pid in common_list:
        df = pd.read_csv(f"/data3/projects/2020_MUTCLUST/Data/Projects/COVID19/Sequence/Raw/Codon/{pid}.codon.csv", index_col=0)
