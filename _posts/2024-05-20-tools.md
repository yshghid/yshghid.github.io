...
#3 kallisto_linux-v0.46.1.tar.gz 파일 경로 설정해주기
```bash
$ vim ~/.bashrc
...
#1 gtf 파일로 생성 Building index with gtf reference file
```bash
...
#2 kallisto 에서 제공하는 유전체 파일로 생성 Building index with kallisto idx file
```bash
...
#### 4) 슈도얼라인 pseudoalign
#1 여기서 중요한 것은 cDNA index (transcripts_cDNA.idx)를 사용한 것. FAQ에 들어가보면 cDNA fasta 파일을 권장한다고 나와있다(We recommend using cDNA fasta, specifically the *.cdna.all.fa.gz files).
