...
3트) kallisto_linux-v0.46.1.tar.gz 파일 경로 설정해주기
```bash
$ vim ~/.bashrc
...
1트) gtf 파일로 생성 Building index with gtf reference file
```bash
...
2트) kallisto 에서 제공하는 유전체 파일로 생성 Building index with kallisto idx file
```bash
...
#위 코드에서는 통계 테스트로 'lrt'를 사용하였는데 이 경우 차등 발현의 크기가 제공되지 않음. 차등 발현의 크기도 제공되는 test type을 사용하려면 ward test 사용하기.
