...
5.
각 유전자에 대한 GO term과 pathway 정보는 임의로 생성했다. Gene Ontology 거리 계산 로직, GO term 비교 로직 또한 실제 로직이 아닌 (오직 결과만을 위한)임시 로직을 사용했다.

```python
# Calculate Gene Ontology Distance #임시 로직
...
# Calculate Pathway Similarity #임시 로직   
...
6.최대 클리크 식별, 클리크 연결성 점수(ConnectivityScore) 계산, CCP(클리크 연결성 프로파일) 식별
