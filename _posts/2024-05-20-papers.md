...
"Paper Study"의 코드는 System Biology 알고리즘 논문을 코드화해서 이를 별도의 데이터 다운로드 없이 돌려 볼 수 있도록 짜여져 있다. 블로그에 작성된 코드는 약간의 축약 버전이므로, 실제로 돌려 보려면 [깃허브][2]에 올라가있는 code.ipynb 를 사용하면 된다. 
