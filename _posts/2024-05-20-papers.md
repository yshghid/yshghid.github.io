...
p-value < 0.05 and q-value(FDR) < 0.1 인 유전자만 네트워크 구성에 사용된다. 네트워크의 에지는 각 노드(유전자)의 발현량의 상관계수에 따라 가중치가 매겨져있다. 
...
def create_graph(filtered_genes, normalized_data, threshold=0.7):
    ...
    for i in range(len(genes)):
        for j in range(i + 1, len(genes)):
            ...
            if abs(weight) > threshold: # (데이터 생성 과정에서 그룹 내 노이즈를 너무 작게 줬는지 모든 유전자 사이에 에지가 존재해버려서, 두 노드 사이의 상관계수 절대값이 0.7을 초과하는 경우에만 에지를 추가했다)
