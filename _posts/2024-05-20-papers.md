...
1. 데이터셋 생성
4개 국가(USA, GER, CHN, SA)의 대장암(CRC) 데이터셋을 생성했다. 4개 데이터셋 모두 행은 유전자(g1-g100) 100개로 구성되어있고 열은 10개 샘플로 구성되었으며 CRC 환자(p1-p5)와 대조군(c1-c5)으로 구성해줬다.
...
3. degree, eccentricity, closeness, betweenness 을 계산해서 각 그래프의 노드 강도(node strength)를 계산한다.
```
