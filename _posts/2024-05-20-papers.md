...
System Biology 알고리즘 논문을 읽다보면 이해를 위해 해당 코드를 돌려 보고 싶다고 자주 생각했다. 하지만 생물학 데이터 특성상 데이터셋이 너무 크고 복잡해서 직접 전처리하여 돌려보기 어렵다. 그리고 그래프 기반 알고리즘들은 설명이나 수식만 읽어서는 해당 방법론이 직관적으로 와닿지 않는다. 운좋게 샘플 데이터와 코드가 모두 주어진다 해도 샘플 데이터 또한 실제 데이터이기 때문에 shape, valuecounts 등을 확인하는 방법으로만 데이터 파악이 가능해서 그 또한 직관적이지 않다고 느꼈다. 

이에 최소한의 조건만 만족하는 작은 데이터를 생성해서 논문의 방법론들을 따라할 수 있는 코드가 있다면 좋을 것으로 생각되어, "Paper Study"의 코드들은 System Biology 알고리즘 논문들의 알고리즘을 코드화해서 이를 별도의 데이터 다운로드 없이 돌려 볼 수 있도록 짜여져 있다. 블로그에 작성된 코드는 약간의 축약 버전이므로, 실제로 돌려 보려면 [깃허브][2]에 올라가있는 code.ipynb 를 사용하면 된다. 
...
위 결과는 US 데이터로 식별한 최대 클리크들과 그들의 연결성 점수이다. 이를 국가마다 수행하고, 공통된 최대 클리크(top common scoring cliques)를 찾아서 그 클리크의 연결성 프로필(connectivity profile for one of the top common scoring cliques)을 국가마다 비교해보면, 즉 동일한 시드에서 시작하는 각 국가의 CCP를 비교하면 공통된 최대 클리크에 대한 클리크 연결성 인사이트를 얻을 수 있다. 해당 논문에서는 클리크 연결 프로필을 다음과 같이 분석했다.
