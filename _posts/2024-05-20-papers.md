...
100 rows × 4 columns
```
p-value < 0.05 and q-value(FDR) < 0.1 인 유전자가 네트워크 구성에 사용된다. 네트워크의 에지는 각 노드(유전자)의 발현량의 상관계수에 따라 가중치가 임의로 매겨져있다. 두 노드 사이의 상관계수 절대값이 0.7을 초과하는 경우에만 엣지를 추가했다. 
```python
...
5.최대 클리크 식별, 클리크 연결성 점수(ConnectivityScore) 계산, CCP(클리크 연결성 프로파일) 식별

클리크 강도가 최대인 클리크가 시드로 선택되고, 클리크 연결성 점수가 계산된다. 
...
해당 논문에서는 USA, GER, CHN 에서 동일한 시드에서 비롯되는 3개의 CCP를 시각화했고, 해당 시드의 연결성 프로파일을 봤을 때 클리크 #17 {EP300, ESR1, SMAD4, SMAD3, CBP, AR, CTNNB1}, 클리크 #39 {BRCA1, EP300, ESR1, SMAD3, SMAD, TP53, SP1}, 클리크 #GC1 {EGFR, GRB2, PIK3R1, PTPN6, SHC, SRC, ESR1}에서 차이를 보였다.
